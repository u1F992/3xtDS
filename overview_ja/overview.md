...
ドキュメントの1.2節[^2]で記載されているように、タッチパネルの読み取りは割り込み方式となっています。Y+端子は通常VCCにあり、タッチが発生するとグランドにプルされ、タッチ割り込みが発生し、サンプリング処理が開始されます。

[^2]: 座標の読み取りが有効かどうかを知るためには、画面に触れているかどうかを検出する方法が必要です。これは、プルアップ抵抗を介してY+に正の電圧（VCC）を印加し、X-にグランドを印加することで可能である。プルアップ抵抗は、タッチスクリーンの総抵抗値（通常は数百オーム）よりかなり大きくする必要があります。タッチがないとき、Y+は正の電圧にプルアップされます。タッチがあると、図1に示すように、Y+はグランドにプルダウンされます。この電圧レベルの変化を利用して、ピンチェンジ割り込みを発生させることができます。
